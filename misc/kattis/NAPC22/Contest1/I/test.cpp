...
struct segment {
  pt a, b, v;
  double intersectx(double x) const { return a.y + (x - a.x) / v.x * v.y; }
  double intersecty(double y) const { return a.x + (y - a.y) / v.y * v.x; }
};
...
enum event_type { DELETE, HIT, ADD };
...
bool operator<(const event& e, const event& f) { return f.p < e.p; }
