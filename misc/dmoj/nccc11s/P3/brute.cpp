...
int main() {
  ...
  for (int i = 1; i < N; i++) {
    if (i < N - 1) {
      for (int j = 0; j < i; j++) {
        for (int k = 0; k < i; k++) {
          if (i-j <= K) madd(dp[i][i][k], dp[i-1][j][k]);
          if (i-k <= K) madd(dp[i][j][i], dp[i-1][j][k]);
          madd(dp[i][j][k], dp[i-1][j][k]);
        }
      }
    }
    else {
      for (int j = 0; j < i; j++) {
        for (int k = 0; k < i; k++) {
          if (i-j <= K && i-k <= K) madd(dp[i][i][i], dp[i-1][j][k]);
        }
      }
    }
  }
