...
int solve() {
  ...
  if (sum < 0) return -1;
    ...
    vector<int> dp(N, 1e9);
    segtree<Seg::T, Seg::op, Seg::e> seg(sz(v));
    F0R(i, N) {
      dp[i] = i + seg.prod(0, A[i]);
      if (B[i] >= 0) ckmin(dp[i], (i == 0 ? 0 : dp[i-1]));
      // cout << i << ": " << dp[i] << endl;

      int idx = i == 0 ? compress[0] : A[i-1];
      seg.set(idx, Seg::op(seg.get(idx), -i + (i == 0 ? 0 : dp[i-1])));
      // DEBUG(-i + (i == 0 ? 0 : dp[i-1]));
    }
    return dp[N-1];
  }
}
