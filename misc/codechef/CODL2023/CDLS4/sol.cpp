...
mint solve() {
  ...
  // DEBUG(x);
  vector<vector<vector<mint>>> dp(2, vector<vector<mint>>(x+1, vector<mint>(K+1, 0)));
  dp[0][0][0] = 1;
  F0R(i, N) {
    F0R(j, min(i, x) + 1) {
      F0R(s, K+1) {
        dp[1][j][s] += dp[0][j][s];
        if (j+1 <= x) dp[1][j+1][min(K, s + A[i])] += dp[0][j][s];
      }
    }
    F0R(j, x+1) {
      F0R(s, K+1) {
  ...
  return dp[0][x][K] * MC::fac(x) * MC::fac(N - x);
}
