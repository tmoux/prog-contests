...
void solve() {
  ...
  vector<int> mark(N);
  auto prep = y_combinator([&](auto prep, int i, int p) -> void {
    if (A[i] > 0) mark[i] = 1;
    for (int j: adj[i]) {
      if (j == p) continue;
      prep(j, i);
  ...
  prep(0, 0);
  auto dfs = y_combinator([&](auto dfs, int i, int p) -> pair<int, mint> {
      int numSum = A[i];
      mint num = 1, den = MC::ifac(A[i]);
      for (int j: adj[i]) {
        if (j == p || mark[j] == 0) continue;
        auto [xj, nxj] = dfs(j, i);
        numSum += xj;
        num *= nxj;
        den *= MC::ifac(xj);
      ...
      int xi = numSum + 1;
      mint nxi = MC::fac(numSum) * num * den;
      return {xi, nxi};
    });

  mint ans = dfs(0, 0).second;
  cout << ans << '\n';
}
