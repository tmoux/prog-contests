...
void solve() {
  int N, M, K; cin >> N >> M >> K;
  map<int, mint> col, row;
  F0R(i, K) {
    int q, x, v; cin >> q >> x >> v;
    if (q == 0) {
      if (!row.count(x)) row[x] = v;
      else row[x] *= v;
    }
    else {
      if (!col.count(x)) col[x] = v;
      else col[x] *= v;
    }
  }
  ...
  auto get_row = [&](int r) -> mint {
    return R1 + mint(r - 1) * S;
  };

  mint ans = mint(N) * R1 + mint(N) * (N - 1) / 2 * S;
  for (auto [r, x]: row) {
    ans += get_row(r) * (x - 1);
  }
  cout << ans << '\n';
