...
int solve_a(int N, const vector<int>& A, const vector<int>& B, const vector<int>& C) {
  ...
  map<ll, int> compress;
  F0R(i, sz(ys)) compress[ys[i]] = i;
  for (auto& y: Y) y = compress[y];
...
void solve() {
  ...
  cout << solve_a(N, A, B, C) << ' ';
  cout << solve_a(N, B, A, C) << ' ';
  cout << solve_a(N, C, A, B) << '\n';
}
