...
mint solve() {
  ...
  FOR(l, 1, N+1) {
    FOR(r, l, N+1) {
      mint numArrays;
      if (K == 1) numArrays = l == r;
      else numArrays = choose(r + K - 3 - l + 1, K - 2);
      for (int x = 1; x <= V; x++) {
        mint numPerms = 0;
        if (x == V) {
          numPerms = choose(N - x, r - 1) * fac(r - 1) * fac(N - r);
        }
        else {
          numPerms = mint(l-1) * choose(N - 1 - x, r - 2) * fac(r - 2) * fac(N - r);
        }
        // cout << l << ' ' << r << ' ' << x << ": " << numPerms << endl;
        ans += numArrays * numPerms;
      }
    }
  }
  return ans;
