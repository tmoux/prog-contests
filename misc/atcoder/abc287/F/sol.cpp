...
vector<mint> f(int i, int p, int took) {
  ...
  for (int j: adj[i]) {
    if (j != p) {
      auto v = f(j, i, 1);
      vector<mint> ndp(sz(res) + sz(v)-1, 0);
      F0R(a, sz(res)) {
        F0R(b, sz(v)) {
          ndp[a + b] += res[a] * v[b];
        }
  ...
  for (int j: adj[i]) {
    if (j != p) {
      auto v = f(j, i, 0);
      vector<mint> ndp(sz(res2) + sz(v)-1, 0);
      F0R(a, sz(res2)) {
        F0R(b, sz(v)) {
          ndp[a + b] += res2[a] * v[b];
        }
  ...
  int T = max(sz(res), sz(res2));
  // while (sz(res) < T) res.push_back(0);
  res.resize(T);
  F0R(i, T) res[i] += i < sz(res2) ? res2[i] : mint(0);
  return res;
}
