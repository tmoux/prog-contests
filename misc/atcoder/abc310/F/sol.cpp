...
int main() {
  ...
  F0R(i, N) {
    mint p = mint(1) / A[i];
    mint np = (A[i] > 10 ? mint(A[i] - 10) / A[i] : mint(0));
    F0R(mask, 1 << 11) {
      dp[i+1][mask] += np * dp[i][mask];
    }
    F0R(mask, 1 << 11) {
      for (int x = 1; x <= min(10, A[i]); x++) {
        int nmask = mask;
        for (int j = 0; j + x <= 10; j++) {
          if (mask & (1 << j)) nmask |= (1 << (j+x));
        }
        dp[i+1][nmask] += p * dp[i][mask];
      }
    }
    // for (int x = 1; x <= min(10, A[i]); x++) {
    //   for (int j = 0; j + x <= 10; j++) {
    //     if (i == 1) {
    //       cout << "updating " << x << ' ' << j << ' ' << x+j << endl;
    //     }
    //     dp[i+1][j+x] += (1 - dp[i+1][j+x]) * dp[i][j] * p;
    //   }
    // }
    // cout << i << ": ";
