...
mint solve(int N, string s, vector<int> A) {
  mint ans = 0;
  vector<mint> W(N);
  F0R(i, N) W[i] = s[i] == 'W';
  vector<mint> S(N), WS(N);
  F0R(i, N) {
    S[i] = W[i] ? 0 : A[i];
    if (i > 0) S[i] += S[i-1];
  ...
  vector<mint> F(N);
  F0R(i, N) {
    mint den = mint(i - 1) * i * (i + 1);
    F[i] = den == 0 ? mint(0) : 2 * MC::fac(N) / den;
  }

  auto c = ntt.conv(W, F);
  F0R(k, N) ans += WS[k] * c[k];
  auto d = ntt.conv(WS, F);
  F0R(k, N) ans -= W[k] * d[k];
  return ans;
};
...
int main() {
  ...
  string s; cin >> s;
  string t = s;
  F0R(i, N) t[i] = s[i] == 'W' ? 'B' : 'W';

  vector<int> A(N);
  F0R(i, N) cin >> A[i];

  mint ans = solve(N, s, A) + solve(N, t, A);
