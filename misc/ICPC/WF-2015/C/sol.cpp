...
int main() {
  ...
  mcSFlow<int, ll> F(2*N+4, source, sink);
  for (int i = 1; i <= N; i++) {
    for (int j = i+1; j <= N+1; j++) {
      int ci; cin >> ci;
      F.add_edge(i, j+N+1, ci, 1);
    }
  }
  for (int i = 1; i <= N; i++) {
    if (i == 1) F.add_edge(source, i, 0, K);
    else F.add_edge(source, i, 0, 1);
  }
  for (int i = 2; i <= N+1; i++) {
    F.add_edge(i+N+1, sink, 0, 1);
  }

  cout << F.minCostMaxFlow().second << '\n';
}
