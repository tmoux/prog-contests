...
struct DP {
  ...
  int dp2;
  DP() {
    dp1a.resize(sz(V), INF);
    dp1b.resize(sz(V), INF);
    dp2 = 0;
  }
  int mindp() {
...
DP dfs(int i, int p) {
  DP dp;
  if (sz(adj[i]) == 1) return dp;

  int minw = 0;
  for (auto [j, w]: adj[i]) ckmax(minw, w);
  for (int W = minw; W < sz(V); W++) {
    dp.dp1a[W] = V[W] * deg[i] + (i == p ? 0 : -V[parEdge[i]]);
  }
  for (int W = parEdge[i]; W < sz(V); W++) {
    dp.dp1b[W] = C * deg[i] + (i == p ? 0 : V[W] - V[parEdge[i]]);
  }
  dp.dp2 += C * deg[i];
  for (auto [j, w]: adj[i]) {
    if (j == p) continue;
