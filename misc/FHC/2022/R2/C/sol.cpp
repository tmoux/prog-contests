...
int solve() {
  ...
  int X = A[0].second;
  F0R(i, N) {
    if (A[i].second < X) L += A[i].first;
    else if (A[i].second == X) E += A[i].first;
  }

  mint total = MC::perm(S, K+1);
  mint num = 0;
  /*
  DEBUG(L);
  DEBUG(E);
  */
  for (int i = 1; i <= K+1; i++) {
    mint before = MC::perm(L, i-1);
    mint now = A[0].first;
    mint next = MC::perm(L + E - i, K + 1 - i);
    num += before * now * next;

    // cout << i << ": " << before << ' ' << now << ' ' << next << " -- " << before * now * next << endl;
  }

  /*
  cout << num << " / " << total << endl;
  cout << (double)num.val/total.val << endl;
  */

  mint ans = num * total.inverse();
  return ans.val;
}

int main() {
  ios_base::sync_with_stdio(false); cin.tie(NULL);
  const int MX = 3000 * 3000 + 5;
  MC::init<MX, MOD>();
  int T; cin >> T;
  for (int tc = 1; tc <= T; tc++) {
    cout << "Case #" << tc << ": " << solve() << '\n';
    cerr << "Finished " << tc << endl;
  }
}
