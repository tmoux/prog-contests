...
/*
** Process each word: go in reverse order
** For each word: count number of untaken leaves (subtree query), if > 0, then decrement, else do nothing
** Keep track of # decremented, if exactly L, then win
** 8 * 10^6 * log(N)
 */
...
int solve() {
  ...
  F0R(i, N) {
    ...
    REP(m) {
      string s; cin >> s;
      tree.words[i].push_back(s);
    }
  }
  tree.init();
  segtree<int, Seg::op, Seg::e> seg(N);
  F0R(i, N) seg.set(i, tree.leaves[i]);
  int L = seg.prod(0, N-1);
  int ans = 0;
  for (const auto& [s, v]: tree.mp) {
    vector<pair<int, int>> taken;
    for (auto i: v) {
      int num = seg.prod(tree.tin[i], tree.tout[i]);
      if (num > 0) {
        seg.set(tree.tin[i], seg.get(tree.tin[i]) - num);
        taken.emplace_back(i, num);
      }
    }
