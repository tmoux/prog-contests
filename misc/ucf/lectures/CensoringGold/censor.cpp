...
int main()
{
    ...
    for (int i = 0; i < a; i++) {
        string b; fin >> b;
        badhashes.push_back({strhash(b),b.size()});
    }
    random_shuffle(badhashes.begin(),badhashes.end());
    ...
    for (ll i = 0; i < s.size(); i++) {
        ...
        for (auto p: badhashes) {
            if (length - (int)p.second >= 0) {
                ll subhash = getHash(length-p.second + 1, length);
                if (subhash == p.first) {
                    length -= p.second;
                    for (ll a = 0; a < p.second; a++) {
                        deq.pop_back();
                    }
                    break;
                }
