...
int f(int l, int r, int i) {
  ...
  res = 0;
  if (i > l) {
    int ii = seg.prod(l, i-1).second;
    ckmax(res, f(l, i-1, ii) + abs(i - ii));
  }
  if (i < r) {
    int ii = seg.prod(i+1, r).second;
    ckmax(res, f(i+1, r, ii) + abs(i - ii));
  }
  return res;
}
...
int main() {
  ...
  int ans = f(0, N-1, seg.prod(0, N-1).second);
  cout << ans << '\n';
}
