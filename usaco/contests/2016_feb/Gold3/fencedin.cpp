...
pair<int,int> findParent(int i, int j) {
	...
	return findParent(parent[i][j].first,parent[i][j].second);
}
...
long long kruskal(const vector<Edge>& edges) {
	long long ret = 0;
	int numEdges = 0;
	for (int i = 0; i < edges.size() && numEdges < (((N+1)*(M+1))-1); i++) {
		Edge e = edges[i];
		pair<int,int> x = findParent(e.vertex[0].first,e.vertex[0].second);
		pair<int,int> y = findParent(e.vertex[1].first,e.vertex[1].second);
		
		if (x != y) {
			ret += e.weight;
			numEdges++;
			makeUnion(e.vertex[0],e.vertex[1]);
		}
...
int main() 
{
	...
	for (int i = 0; i <= N; i++) {
		for (int j = 0; j <= M; j++) {
			if (i != N) {
				Edge e;
				e.vertex[0] = make_pair(i,j);
				e.vertex[1] = make_pair(i+1,j);
				if (j == 0) {
