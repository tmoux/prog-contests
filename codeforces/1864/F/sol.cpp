...
int main() {
  ...
  segtree<int, Max::op, Max::e> segmax(N);
  vector<map<int, int>> remove(N+1);
  for (const auto& [x, vec]: v) {
    for (int i = 1; i < sz(vec); i++) {
      int mx = segmax.prod(vec[i-1], vec[i]);
      if (mx > 0) {
        remove[mx][x]++;
      }
    }
  ...
  for (auto [l, r, id]: queries) {
    while (x >= l) {
      if (sz(v[x])) seg.set(x, seg.get(x) - (sz(v[x]) - 1));

      for (auto [y, d]: remove[x]) {
        seg.set(y, seg.get(y) + d);
      }
