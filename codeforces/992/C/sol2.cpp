...
public:
    ...
    BigInteger(ll n); // "int" constructor
...
BigInteger modexp(BigInteger x, BigInteger n) {
    if (n == 0) return 1;
    if (n == 1) return x % MOD;
...
int main()
{
    ...
    int c = (k % 2 ? -1 : 1);
    BigInteger ans = ((modexp(2,ka+1) * xa) % MOD - (modexp(2,ka) - c)) % MOD;
    cout << ans << '\n';
...
BigInteger::BigInteger(ll n) { // "int" constructor
    stringstream ss;
    string s;
    ss << n;
    ss >> s;
