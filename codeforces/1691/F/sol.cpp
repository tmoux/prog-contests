...
int main() {
  ...
  auto get_subtree = [&](int i, int j) {
    return j == par[i] ? N-sub_size[i] : sub_size[j];
  };
  dfs(1, 1);
  ...
  for (int i = 1; i <= N; i++) {
    mint X = 0;
    for (int j: adj[i]) {
      int s = get_subtree(i, j);
      X += MC::choose(s, K);
    }
    for (int j: adj[i]) {
      int roots = get_subtree(i, j);
      int sz = N-roots;

      X -= MC::choose(roots, K);
      ans += mint(sz) * roots * (MC::choose(sz, K) - X);
      X += MC::choose(roots, K);
    }
    ans += mint(N) * (MC::choose(N, K) - X);
  }
  cout << ans << '\n';
}
