...
struct Tracker {
  ...
  void insert(int i, int x) {
    if (x < getVal(i)) {
      auto it = s.insert({i, x}).first;
      if (next(it)->first == i)
        s.erase(next(it));
      while (it != s.begin()) {
        if (prev(it)->second >= x) s.erase(prev(it));
        else break;
      }
    }
  }
...
int main() {
  ...
  F0R(r, N) {
    seg.set(A[r], maxn);
    for (auto l: updates[r]) {
      int x = tracker.getVal(l);
      if (r > l) ckmin(x, abs(A[r] - A[l]));
      tracker.insert(l, x);

      int idx = seg.prod(max(1, A[l]-x+1), min(N, A[l]+x-1));
      if (idx <= N) updates[idx].push_back(l);
    }

    for (auto [l, id]: queries[r]) {
      ans[id] = tracker.getVal(l);
    }
  }
