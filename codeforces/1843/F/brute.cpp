...
struct Tree {
  ...
  vector<int> tin, tout;
  vector<int> A;
  vector<int> minPfx, maxPfx;
  vector<int> minRange, maxRange;

  void dfs(int i, int p, int cur, int& t) {
    tin[i] = t++;
    depth[i] = depth[p] + 1;
    // par[0][i] = p;
    minPfx[i] = min(minPfx[p], cur);
    maxPfx[i] = max(maxPfx[p], cur);
    int ncur = cur + A[i];
    minRange[i] = min(minRange[p], ncur - maxPfx[i]);
    maxRange[i] = max(maxRange[p], ncur - minPfx[i]);
...
void solve() {
  ...
  for (auto [u, v, k]: queries) {
    assert(u == 1);
    int mn = tree.minRange[v];
    int mx = tree.maxRange[v];
