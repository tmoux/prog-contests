...
int main() {
  ...
  using segtree = segtree<int, Seg::op, Seg::e>;
  segtree seg(N+1);
  auto add_range = [&](int i, int x) -> void {
    int l = tree.tin[i], r = tree.tout[i];
    seg.set(l, seg.get(l) + x);
    seg.set(r+1, seg.get(r+1) - x);
  };
  auto getsum = [&](int i) -> int {
    return seg.prod(0, tree.tin[i]);
  };
  vector<segtree> segs(2, segtree(N));
