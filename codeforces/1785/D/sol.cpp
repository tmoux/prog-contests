...
int main() {
  ...
  for (int l = N; l >= 0; l--) {
    for (int i = 0; i < (1 << N); i++) {
      if (l == 0) {
        dp[1][i] = i == 0 ? mint(0) : 2 * dp[0][i-1];
      }
      else {
        int K = (1 << N) - (i + 1) - (1 << (l-1));
        int M = (1 << (l-1)) - 1;
        mint arrange = MC::choose(K, M) * MC::fac(1 << (l-1));
        mint fill_rest = l == N ? mint(1) : (i == 0 ? mint(0) : 2 * dp[0][i-1]);
        dp[1][i] = arrange * fill_rest;
      }
  ...
  F0R(i, 1 << N) {
    mint ans = dp[0][i] - (i == 0 ? mint(0) : dp[0][i-1]);
    cout << ans << '\n';
  }
}
