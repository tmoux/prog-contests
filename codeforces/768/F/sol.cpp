...
using MC::strong_compositions;
using MC::weak_compositions;
...
int main() {
  ...
  for (int k = 1; k <= X + Y; k++) {
    ...
    mint add = 0;
    REP(2) {
      add += strong_compositions(X, a) * strong_compositions(Y, b);
      swap(a, b);
    }
    total += add;
  }

  cout << MC::weak_compositions(0, 0) << endl;
  ...
  for (int k = 1; k <= X + Y; k++) {
    ...
    REP(2) {
      if (Y - 1LL * H * b >= 0) {
        num += strong_compositions(X , a) *
               strong_compositions(Y - H * b, b);
      }
      swap(a, b);
    }
  }
