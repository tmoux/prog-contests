...
template<typename T>
Matrix<T> matpow(Matrix<T> M, ll p) {
  assert(M.n == M.n);
  Matrix<T> res(M.n, M.n);
  F0R(i, M.n) res.A[i][i] = 1;
  while (p) {
    if (p&1) res = res * M;
    M = M * M;
    p >>= 1;
...
mint solve() {
  int n, x, k; cin >> n >> x >> k;
  mint total_ways = mint(x + k) * (mint(2 * k + 1) ^ (n - 1));

  Matrix<mint> A(x, x);
  F0R(i, x) F0R(j, x) A.A[i][j] = abs(i - j) <= k;
  A = matpow(A, n-1);
  mint bad_ways = 0;
  F0R(i, x) F0R(j, x) bad_ways += A.A[i][j];

  return total_ways - bad_ways;
}
