...
namespace ModCombinatorics {
  ...
  mint choose_inv(int n, int k) {
    if (n < k || k < 0) return 0;
    return mint(1) * _ifac[n] * _fac[k] * _fac[n-k];
  }
...
mint solve(int N, int K) {
  mint ans = 0;
  for (int m = 1; m < N; m++) {
    ll numNeed = m + 1LL * (m - 1) * (K - 1);
    if (numNeed > N) break;
    int numExtra = N - numNeed;
    mint numArrangements = MC::weak_compositions(m, numExtra + 1);
    mint prob = MC::choose_inv(N, m);
    ans += numArrangements * prob;
  }
  return ans + 1;
}
