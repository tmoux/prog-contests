...
int main() {
    ...
    while (q--) {
        ...
        auto it = curr.find(u);
        if (it != curr.end()) {
            //delete: remove 2 current, add 1 bigger
            int l = *prev(it);
            int r = *next(it);
            ans = madd(ans,M-root->getans(l,u-1));
            ans = madd(ans,M-root->getans(u,r-1));
            ans = madd(ans,root->getans(l,r-1));
            curr.erase(it);
        }
        else {
            //add: remove 1 big, add 2 smaller
            auto it2 = curr.upper_bound(u);
            int l = *prev(it2);
            int r = *it2;
            ans = madd(ans,M-root->getans(l,r-1));
            ans = madd(ans,root->getans(l,u-1));
            ans = madd(ans,root->getans(u,r-1));
            curr.insert(u);
        }
        cout << ans << '\n';
    }
