...
int main() {
  ...
  if (K == 0) {
    ...
    mint ans = 0;
    for (int m = 1; m <= K; m++) {
      // Case 1: run starts at beginning
      if (N - (2*m-1) >= 0) {
        mint add = MC::strong_compositions(K, m);
        add *= MC::weak_compositions(N-K-(m-1), m);
        add *= mint(3) ^ (N - (2*m-1));
        ans += add;
      }

      // Case 2: run does not start at beginning
      if (N - 2*m >= 0) {
        mint add = MC::strong_compositions(K, m);
        add *= MC::weak_compositions(N-K-m, m+1);
        add *= mint(3) ^ (N - 2*m);
        ans += add;
      }
    }
