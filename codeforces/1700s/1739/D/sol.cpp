...
struct Tree {
  ...
  void dfs(int i, int p, int& t) {
    tin[i] = t++;
    verts.push_back(i);
    // assert(0 <= i && i < sz(depth) && 0 <= p && p < sz(depth));
    // DEBUG(p);
    // DEBUG(depth[p]);
    // DEBUG(sz(depth));
    depth[i] = depth[p] + 1;
...
int solve() {
  ...
  auto check = [&](int x) -> bool {
    F0R(i, N) seg->add(i, i, -seg->getsum(i, i));
    int total = 0;
    for (auto i: tree.verts) {
      // cout << tree.tin[i] << ' ' << tree.tout[i] << endl;
      int t = tree.tin[i];
      if (tree.depth[i]-1 > x && seg->getsum(t, t) == 0) {
        int j = tree.getpar(i, x-1);
        seg->add(tree.tin[j], tree.tout[j], 1);
        total++;
      }
    }
    // cout << x << ": " << total << endl;
