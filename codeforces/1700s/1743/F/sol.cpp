...
int main() {
  ...
  const int MX = 300000;
  vector<vector<int>> starts(MX+1), ends(MX+1);
  pair<int, int> firstInterval;
  cin >> firstInterval.first >> firstInterval.second;
  for (int i = 1; i < N; i++) {
    int l, r; cin >> l >> r;
  ...
  mint total = 0;
  segtree<Matrix, Seg::op, Seg::e> seg(N);
  F0R(i, N) seg.set(i, A);
  for (int x = 0; x <= MX; x++) {
    for (int i: starts[x]) {
      seg.set(i, B);
    ...
    Matrix M = seg.prod(1, N-1); 
    int index = (firstInterval.first <= x && x <= firstInterval.second) ? 1 : 0;
    mint ans = M.a[1][index];
    total += ans;
