...
int main() {
  ...
  F0R(i, N-1) {
    ...
    tree.add_edge(a, b);
    ADJ[a].push_back({b, i});
    ADJ[b].push_back({a, i + (N-1)});
  }
  tree.init();

  auto other = [&](int e) {
    return e < N-1 ? e + (N-1) : e - (N-1);
  };
  ...
  auto rebuild = [&]() -> void {
    rebuilds++;
    // cout << "rebuilding " << queries << endl;
    vector<vector<int>> add(N, vector<int>(MX+1));
    queue<array<int, 3>> q[MX+1];
    for (auto [u, v, lca, k, d]: queries) {
      add[u][d] += k;
      add[v][d] += k;
      add[lca][d] -= 2 * k;
      q[d].push({lca, -1, k});
