...
// Count contribution of each prime individually.
// For each prime and each edge, count the # of triples that cross this edge.
// To speed this up, build a virtual tree for each edge. Max number of nodes is around 3e6, maybe x2.
...
int main() {
  ...
  F0R(i, N) {
    ...
    for (auto p: v) primes[p].push_back(i);
  }
  Tree tree(N);
  REP(N-1) {
    int a, b; cin >> a >> b;
  ...
  for (int p = 2; p < maxn; p++) {
    if (!primes[p].empty()) {
      sort(all(primes[p]), [&](int a, int b) {
        return tree.tin[a] < tree.tin[b];
      });
      vector<int> v = primes[p];
      F0R(idx, sz(primes[p])-1) {
        int a = primes[p][idx];
        int b = primes[p][idx+1];
        v.push_back(tree.lca(a, b));
      }
      sort(all(v)); v.erase(unique(all(v)), v.end());
      ...
      stack<int> st;
      for (auto x: v) {
        while (!st.empty() && tree.tout[st.top()] < tree.tin[x]) st.pop();
        if (!st.empty()) {
          int par = st.top();
          adj[par].push_back({x, tree.depth[x]-tree.depth[par]});
        }
        st.push(x);
      }
      int total = sz(primes[p]);
      const int inv2 = mint(1) / 2;
      auto calc = [&](int x) -> mint {
        auto f = [&](int x, int y) {
          return mint(x) * (x - 1) * y * inv2;
        };
        return f(x, total - x) + f(total - x, x);
      };
      for (auto x: primes[p]) in[x] = 1;
      y_combinator([&](auto dfs, int i) -> int {
        int s = in[i];
        for (auto [j, w]: adj[i]) {
          int t = dfs(j);
          s += t;
