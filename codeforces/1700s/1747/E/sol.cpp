...
namespace ModCombinatorics { // {{{
  ...
  template<size_t N, int MOD>
    void init() {
      ...
      pow2[0] = 1;
      for (size_t i = 1; i < N; i++) pow2[i] = mint(2) * pow2[i-1];
    }
  ...
  mint weakComps(int n, int k) {
    return choose(n+k-1, k-1);
  }
  
  mint strongComps(int n, int k) {
...
int solve() {
  ...
  for (int k = 1; k <= N; k++) {
    mint num = MC::pow2[M];
    num *= mint(4)*k*k + mint(4)*k*(1+M)+mint(M)*(3+M);
    num *= MC::fac(M+k-1);
    mint denom = MC::inv4 * MC::ifac(k) * MC::ifac(M);
    mint add = num * denom;

    add *= MC::strongComps(N, k);
    ans += add;
  }
  return ans.val;
}
