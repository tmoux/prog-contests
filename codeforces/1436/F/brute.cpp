...
mint G(int n) {
  if (n < 1) return 1;
  return (n+2) * (mint(2) ^ (n - 1));
}

mint calc(ll N, mint sumsq, mint sumprod) {
  mint ans = 0;
  cout << "calc " << N << ' ' << sumsq << ' ' << sumprod << endl;
  ans += sumsq * F(N-1);
  cout << "1 " << F(N-1) << endl;
  ans += sumprod * G(N-2);
  cout << "2 " << G(N-2) << endl;
...
int main() {
  ...
  int N = 0;
  REP(M) {
    int a, freq; cin >> a >> freq;
    REP(freq) v.push_back(a);
    N += freq;
  }
  ...
  mint sumsq = 0, sumprod = 0;
  F0R(i, N) sumsq += v[i] * v[i];
  F0R(i, N) {
    FOR(j, i+1, N) {
      sumprod += mint(2) * v[i] * v[j];
    }
  }
  cout << calc(N, sumsq, sumprod) << endl;
  ...
  //     if (i == j) {
  //       for (int k = 0; k <= N-1; k++) {
  //         cnt += MC::choose(N-1, k) * k;
  //       }
  //     }
  //     else {
  //       for (int k = 0; k <= N-2; k++) {
  //         cnt += MC::choose(N-2, k) * (k+1);
  //       }
  //     }
  //     ans2 += mint(v[i]) * v[j] * cnt;
  //   }
  // }
  // for (auto x: v) {
  //   ans2 += mint(x) * (sum - x) * (N-1) * (mint(2) ^ (N-2));
  // }
  // cout << ans2 << endl;
}
