...
int main() {
  ...
  // https://oeis.org/A011973 
  auto nonConsecutive = [&](int n, int k) {
    return mint(MC::choose(n-k+1, k)) - MC::choose(n-k-1, k-2);
  };
  mint ans = 0;
  for (int gaps = n&1; 2*gaps <= n; gaps += 2) {
    int k = n-gaps;
    mint gapChoices = nonConsecutive(n, gaps);
    mint add = 2 * MC::fac(k) * gapChoices;
    ans += add;
  }
  cout << ans << '\n';
}
