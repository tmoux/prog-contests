...
bool solve() {
  ...
  // segtree<int, Seg::op, Seg::e> seg(MX);
  // map<int, int> mp;

  // vector<int> cnt(N);
  // F0R(i, N) {
  //   int x = seg.prod(A[i]+1, MX-1);
  //   cnt[i] ^= x&1;
  //   seg.set(A[i], seg.get(A[i]) + 1);
  // }
  // F0R(i, MX) seg.set(i, 0);
  // F0Rd(i, N) {
  //    int x = seg.prod(0, A[i]-1);
  //    cnt[i] ^= x&1;
  //    seg.set(A[i], seg.get(A[i]) + 1);
  // }
  ...
  // for (auto [x, vec]: v) {
  //   F0R(idx, sz(vec)) {
  //     if (cnt[vec[idx]] == 0) continue;
  //     else {
  //       if (idx == sz(vec)-1) return false;
  //       else if (cnt[vec[idx+1]] != 1) return false;
  //       else idx++;
  //     }
  //   }
  // }
