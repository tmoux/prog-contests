...
bool solve() {
  ...
  if (K == 1) {
    F0R(i, N) if (i != B[i]) return false;
    return true;
  }
  // check that there are no 1-cycles
  F0R(i, N) if (i == B[i]) return false;
  auto [ngraph, compOf] = graph.createCondensationGraph();
  // Claim: possible iff all cycles are exactly length k
  vector<int> szcomp(ngraph.n);
  F0R(i, N) {
    szcomp[compOf[i]]++;
  }
  F0R(i, N) {
    int c = compOf[i];
    if (szcomp[c] > 1 && szcomp[c] != K) return false;
  }
  return true;
}
