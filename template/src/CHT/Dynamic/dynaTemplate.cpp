...
struct FN
{
    ...
    friend auto maxcmp(const FN& rhs) const {
        if (!rhs.is_query) return m < rhs.m;
        ll x = rhs.m;
        return xRight() < (ld)x;
    }

    friend auto mincmp(const FN& rhs) const {
        if (!rhs.is_query) return m > rhs.m;
        ll x = rhs.m;
        return xRight() < (ld)x;
    }
...
template<typename cmp> //cmp is a lambda comparator
struct Hull: public set<FN,decltype> //convex hull for maximum
{
    void addFN(FN fn) {
